...
M1_TAIL = (
    "No number in this report was measured on\n"
...
LABELS_KEY = (
    "Labels: [measured] instrument/benchmark on record - [derived] arithmetic from "
...
LEVER_NAME = {
    "vram_bw": "faster GPU memory bus",
    "io": "faster storage / NVMe",
...
_WRAP = {"break_long_words": False, "break_on_hyphens": False}
...
def _parse_bench_log(path, quiet=False):
    ...
    out = {"rows": [], "build": None, "size_gib": None, "params_b": None, "path": path}
    for line in text.splitlines():
        ...
        if len(cells) < 3:
            continue
        test = next(
            (c for c in cells if re.fullmatch(r"(?:pp|tg)\d+(?:\s*@?\s*d\d+)?", c)),
            None,
        )
        if not test:
            continue
        ...
        out["rows"].append({"test": test, "tok_s": float(m.group(1)), "err": float(m.group(2))})
    if not out["rows"] and not quiet:
        print(
            f"[quantprobe] --bench-log {path}: no llama-bench result rows found - "
...
def _bench_context(bench, px):
    ...
    if pb is None:
        mismatch = (
            "the log carries no parseable params column, so it cannot be verified "
            "as THIS model - no predicted-vs-measured ratio is quoted"
        )
    elif t and abs(pb - t) / t > BENCH_PARAM_TOL:
        mismatch = (
            f"the log reports {pb:g}B params where this file is {t:.1f}B - a "
...
def _verdict(px, bctx, gguf_base):
    ...
    if best[0] == "all in VRAM":
        ...
        L.append(
            _lab(
                f"{'PREDICTED decode speed, one user:':<36}{_tps(tps)} tok/s - a FLOOR",
                "[measured] one-sided",
            )
        )
        L += _para(
            "Real speed measured >= 0.90x this number in 13 of 13 benchmarks, and in "
        ...
        L.append(
            _lab(
                f"{'PREDICTED decode speed, one user:':<36}{_tps(tps)} tok/s   "
                f"(band {_tps(tps * 0.75)} - {_tps(tps * 1.25)})",
                "[derived]",
            )
        )
    if bctx and bctx["decode"] and not bctx["mismatch"]:
        ...
        L.append(
            _lab(
                f"{'MEASURED on this machine:':<36}{d['tok_s']:g} +/- {d['err']:g} "
                f"tok/s  (llama-bench)",
                "[measured]",
            )
        )
        if bctx["ratio"]:
            ...
            verdict = (
                f"inside the +/-25% band{side}."
                if bctx["inband"]
                else "OUTSIDE the +/-25% band - trust the measurement, not the prediction; "
...
def _fit_sentences(px):
    ...
    if name.startswith("hybrid"):
        return (
            f"Fit: the {size:.1f} GB file does not fit this machine's {vc:g} GB VRAM "
    ...
    return (
        f"Fit: the {size:.1f} GB file exceeds this machine's memory tiers ({vc:g} GB "
...
def _asked(args, px, gguf_base):
    ...
    L += _kv(
        "model",
        f"{inp['t']:.1f}B params, {inp['a']:.1f}B active/token, {kind}, "
    ...
    if inp["ctx"] > 0:
        kv_txt += (
            f"; ctx {inp['ctx']}: +{inp['ctx'] * inp['kvp'] / 1e9:.2f} GB KV read "
...
def _machine_lines(args, px):
    ...
    consts = (
        f"VRAM {hw['vc']:g} GB @ {hw['vb']:g} GB/s | RAM {hw['rc']:g} GB @ "
    ...
    if preset:
        ...
        L += _kv(
            "calibration",
            "not consulted: a preset machine was named, so these are the "
    ...
    if hint is None:
        ...
        L += _kv(
            "machine",
            f"described by explicit hardware flags; {consts}",
            "[user-supplied]",
        )
        L += _kv(
            "calibration",
            "not consulted: explicit flags describe a different box than "
            "the one generating this report.",
        )
        return L
    L += _kv(
        "machine",
        f"this machine, auto-detected; {consts}",
    ...
    if cal and hw["cal_active"]:
        ...
        L += _kv(
            "calibration",
            f"applied, measured {cal.get('date', '?')} on this box, "
            f"state {cal.get('cal_id', '?')}{stale}",
            "[measured]",
        )
        ...
        L += _kv(
            "calibration",
            "none on file: constants above are OS-reported/auto-detected, "
    ...
    if gr or cr:
        parts = [
            p
            for p in (
                f"GPU x{gr:.2f}" if gr else None,
                f"CPU x{cr:.2f}" if cr else None,
            )
            if p
        ]
        L += _kv(
            "anchors",
            "tier ratios from your own calibrate anchor runs: " + ", ".join(parts),
            "[measured]",
        )
    return L
    ...
    for i, row in enumerate(px["rows"]):
        ...
        if not runnable and not sep_done:
            ...
            L += [
                "  " + ln
                for ln in _para(
                    "rows below need a runtime llama.cpp does not "
...
def _limits(px):
    ...
    if not bc:
        return _para(
            "This placement carries no time decomposition, so nothing is classified "
    ...
    scope_note = _para(
        "Validation: this classification is derived from the law, not "
    ...
    if cap:
        ...
        head = _para(
            f"CAPACITY-BOUND ({cap['tier']}): this configuration is "
        ...
        if cap["gain_shave"] >= planmod.CAP_PROMOTION_MIN:
            L.append(
                _lab(
                    f"  cross it by   shaving the file (next quant tier down)"
                    f"  -> ~{_tps(cap['shave_tps'])} tok/s",
                    "[derived]",
                )
            )
            ...
            L.append(
                _lab(
                    f"           or   fitting {cap['need_gb']:.1f} GB of {cap['tier']}"
                    f"          -> ~{_tps(cap['lift_tps'])} tok/s",
                    "[derived]",
                )
            )
            ...
            L.append(
                _lab(
                    f"  cross it by   fitting {cap['need_gb']:.1f} GB of {cap['tier']}"
                    f"  -> ~{_tps(cap['lift_tps'])} tok/s",
                    "[derived]",
                )
            )
            L += [
                "                " + ln
                for ln in _para(
                    f"(shaving the file does not pay here: x{cap['gain_shave']:.1f})",
                    COL - 16,
                )
            ]
        if cap.get("kv_closes"):
            L.append("  KV lever      quantize the KV cache (-ctk q8_0 -ctv q8_0): frees")
            L.append(
                f"                {cap['kv_saved_gb']:.1f} GB of the {cap['kv_gb']:.1f} "
                f"GB f16 cache and crosses the"
            )
            L.append(
                _lab(
                    f"                boundary WITHOUT touching the weights"
                    f"  -> ~{_tps(cap['kv_tps'])} tok/s",
                    "[derived]",
                )
            )
            body = _para(
                "Prefer this at depth over dropping a quant tier. Both halves are "
            ...
            body = _para(
                f"KV lever: quantizing the KV cache (-ctk q8_0 -ctv q8_0) returns "
        ...
        L.append(
            _lab(
                f"  until then    {planmod.RESOURCE_LABEL[bc['resource']]} is where the "
                f"time goes - {_pct(bc['share'])} of every token",
                "[derived]",
            )
        )
    else:
        L.append(
            _lab(
                f"{bc['klass'].upper()} ({bc['label']}): {_pct(bc['share'])} of every "
                f"decode token is spent there.",
                "[derived]",
            )
        )
        L += scope_note
        if bc.get("margin_x") and bc.get("next_resource"):
            L += [
                "  " + ln
                for ln in _para(
                    f"margin: {planmod.RESOURCE_LABEL[bc['resource']]} must get "
    ...
    if alts:
        L.append(
            _lab(
                "Upgrades that WOULD move it (the law re-run with one input improved):",
                "[derived]",
            )
        )
        for u in alts:
            L.append(
                f"  {u['name']:<27} -> ~{_tps(u['tps'])} tok/s (x{u['gain']:.2f}, "
                f"lands on: {u['row']})"
            )
        L.append("")
    ...
    if wont:
        L.append(
            _lab(
                "Levers that will NOT help here (exact ceilings, not opinions):",
                "[derived]",
            )
        )
        L += wont
...
def _serving_note(placement):
    ...
    if placement == "all in VRAM":
        return (
            "Multi-user, measured on one Pascal-class box (a 7B Q4 dense model in VRAM): "
    ...
    return (
        "Multi-user, measured on one Pascal-class box: batching does NOT survive this "
...
def _depth_note(px):
    ...
    if not planmod.depth_scope_warning(px["best"][0], inp["moe"], inp["ctx"]):
        return None
    return (
        f"Depth note: this is a dense split at {inp['ctx']} context, where the "
...
def _command(args, px, gguf_base):
    ...
    if "--threads" in flags:
        ...
        L += [
            "  " + ln
            for ln in _para(
                f"--threads {nthreads} = the report-generating machine's logical cores. "
                f"llama.cpp's own auto-detect may pick physical-only and cost 2x on "
                f"CPU-bound decode - verify on your build if unsure.",
                COL - 2,
            )
        ]
...
def _optional_lever_note(px):
    ...
    if not moe and "layers->VRAM" in name:
        return (
            (
                "Optional lever, measured on this placement class (not on this file): a "
    ...
    if not moe and name == "all in VRAM":
        return (
            (
                "Optional lever, measured on this placement class (not on this file): on "
                ...
                "newer GPUs the kernel boundary is unverified."
            ),
            "[measured, other model]",
        )
    if moe and "split experts" in name:
        ...
        base = (
            "Optional lever, measured on this placement class: if output REUSES its "
        ...
        if reach and hx and reach < hx * (1.0 - planmod.SPEC_HEADLINE_TOL):
            base += (
                f" On THIS row the same drafter is capped at {reach:.2f}x by the row's "
    ...
    if moe and "exps=CPU" in (best[3] or ""):
        return (
            (
                "Speculation note, measured on this placement class: external-draft "
...
def _measured_check(px, bctx, gguf_base, machine_flag):
    ...
    if bctx:
        ...
        if b["size_gib"] and px["size"]:
            ...
            if abs(gb - px["size"]) / px["size"] < 0.02:
                L.append("")
                L += _para(
                    f"(bench reports the file as {b['size_gib']:.2f} GiB = {gb:.1f} "
    ...
    if hw["gpu_ratio"] or hw["cpu_ratio"]:
        parts = [
            p
            for p in (
                f"GPU x{hw['gpu_ratio']:.2f}" if hw["gpu_ratio"] else None,
                f"CPU x{hw['cpu_ratio']:.2f}" if hw["cpu_ratio"] else None,
            )
            if p
        ]
        body = _para(
            "No llama-bench log was supplied, but this machine's own calibration "
        ...
        L += body
        L += _para(
            f"One command adds a direct check of this file: `quantprobe bench "
...
def _quality(args, px, rec):
    ...
    L += _kv(
        "this file",
        f"{inp['bits']:.2f} bits/weight -> est. quality cost "
    ...
    if rec:
        ...
        L += _kv(
            "fragile band",
            f"measured for THIS architecture ({mm['name']}): layers "
        ...
        L += _kv(
            "fragile band",
            f"no measured recipe for this architecture ({arch}; {n} "
    ...
    L.append("")
    L += _para(
        "Capability numbers above were measured on other models (Qwen3.5-35B/4B, "
...
def _size_law(t):
    ...
    if t >= 30:
        lines = _kv(
            "size law",
            f"at {t:.0f}B this model is in the size class where a "
            f"measured recipe plus aggressive 2-bit PAYS: on a 35B, "
            f"protected 2-bit held MATH-500 at 81.0 vs 57.0 for the "
            f"naive 2-bit quant.",
            "[measured, Qwen3.5-35B]",
        )
        if t >= 45:
            lines += _kv(
                "",
                "Above the 35B measured point the direction is unmeasured - stated, not assumed.",
                "UNVALIDATED",
            )
        return lines
    if t <= 8:
        return _kv(
            "size law",
            f"at {t:.1f}B this is a SMALL model, and 2-bit is a false "
...
def _shave_price(px, rec):
    ...
    L = _kv(
        "the shave",
        f"crossing the {cap['tier']} boundary needs "
    ...
    if implied < 3.5:
        ...
        cmd = (
            f"quantprobe quantize --gguf <higher-precision source> --recipe {key}"
            if key
            else "quantprobe probe --gguf <higher-precision source> --eval wiki.test.raw --apply"
        )
        L += _kv(
            "",
            f"Below ~3 bits at this size, capability is where the risk lives (see "
            ...
            f"scored on your own tasks.",
            "UNVALIDATED below ~3 bits",
        )
    return L
...
def _scope(args, px):
    ...
    L.append("- Speeds are decode (writing). Prompt reading is a different regime and is not")
    L.append("  classified here.")
    L.append("- Every speed marked PREDICTED is a prediction, not a guarantee: the stated band")
    ...
    L.append("  deciding test for a business decision is your own workload; a run-your-own-tasks")
    L.append("  mode is planned, not shipped.")
    if getattr(args, "machine", None):
        L.append("- Machine constants are the named preset's, measured or estimated on the box")
        L.append("  that preset describes - not on the reader's machine.")
    else:
        L.append("- Machine constants describe the machine that GENERATED this report - not the")
...
def _reproduce(snap, px, rec):
    ...
    L += _para(
        f"Sources: Law 4 (tok/s = eta x bandwidth / active bytes); placement engine, "
...
def render(args, px, bctx, snap, rec):
    ...
    machine_flag = f" --machine {snap['machine']}" if snap.get("machine") else ""
    where = snap.get("machine") or (
        "this machine" if px["inputs"]["machine_hint"] else "the described machine"
    )
    doc = []
    doc.append(f"# quantprobe report - {gguf_base} on {where}")
    doc.append("")
    doc.append(f"Generated {time.strftime('%Y-%m-%d')} by quantprobe v{__version__} (report v1).")
...
def run(args):
    ...
    if not g or not os.path.isfile(g):
        raise SystemExit(
            f"report needs the file it is about: --gguf {g or '<missing>'} "
    ...
    if not hasattr(args, "arch"):
        ...
        raise SystemExit(
            "report is grounded in the GGUF, and autospec could not read this "
    ...
    if best[0] == "all in VRAM":
        ...
        v = (
            f"PREDICTED {_tps(best[1])} tok/s decode, one user "
            f"(band {_tps(best[1] * 0.75)}-{_tps(best[1] * 1.25)}) - {best[0]}"
        )
    if bctx and bctx["ratio"]:
        mline = (
            f"measured: llama-bench {bctx['decode']['tok_s']:g} tok/s -> ratio "
    ...
    if rec:
        ...
        qline = (
            f"quality: {px['inputs']['bits']:.2f} bits/weight, est. cost "
    ...
    print(f"  {mline}")
    print(f"  {qline}")
    print("  forward the FILE, not a number from it - its labels, bands and scope travel inside.")
