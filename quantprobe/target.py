...
CATALOG.update(
    {
        "qwen3-4b": {"t": 4.0, "a": 4.0, "ne": 4.0, "moe": False, "hint": "Qwen3-4B"},
        "qwen3-14b": {
            "t": 14.8,
            "a": 14.8,
            "ne": 14.8,
...
def feasible(a, tps_target):
    ...
    _hw, vc, vb, rc, rb, db, geta, gl = hw_of(a)
    rows = []
    for key, m in CATALOG.items():
        for bits in BITS_LADDER:
            size, _act, cfgs = planmod.evaluate(
                m["t"],
                m["a"],
                m["ne"],
                m["moe"],
                ...
                gl,
                ctx=ctx,
                kvp=m.get("kvp", planmod.DEFAULT_KVP),
                n_layer=m.get("nl"),
            )
            best = cfgs[0]
...
def run(a):
    ...
    hw, _vc, _vb, _rc, _rb, _db, _geta, _gl = hw_of(a)
    ...
    if not rows:
        ...
        print(
            f"    {w['tps']:.1f} tok/s | {w['size']:.1f} GB | quality x{w['q']:.2f} | {w['place']}"
            + (f"  [{w['warn']}]" if w["warn"] else "")
        )
        print(
            f"    run it:  quantprobe run --gguf <{w['key']}.gguf> --model {w['key']}"
        ...
        if len(rows) > 1:
            ...
            for r in rows[1:6]:
                print(
                    f"    {r['tps']:6.1f} tok/s  {r['hint']:22s} @ {r['bits']:g}-bit"
    ...
    if a.ladder:
        ...
        for band in [30, 20, 10, 5, 2, 0.5]:
            ...
            if rr:
                w = rr[0]
                print(
                    f"  {band:>5g}+   {w['hint'] + ' @ ' + format(w['bits'], 'g') + '-bit (' + format(w['t'], 'g') + 'B)':40s}"
                    f" {w['tps']:6.1f} tok/s"
                )
            else:
                print(f"  {band:>5g}+   {'-':40s}")
