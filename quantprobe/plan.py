...
def run(args):
    ...
    if ph:
        ...
        if chat["label"] != rag["label"]:
            ...
            print()
            print("  honesty on GENERATION speed (preregs #43/#60): a plain fixed -ngl split measures")
            print("  EQUAL tg to this -ot placement - twice, weeks apart. The -ot advice earns its keep")
            print("  on PROMPT PROCESSING (2.2x measured), on keeping KV in VRAM, and on enabling the")
            print("  speculation numbers below - not on raw generation. And the tg above is a COLD-BOX")
            print("  number: after ~9h of sustained load the same box measured 15.3 (-29%), outside our")
            print("  own +/-25% band (prereg #60, runaway-process contamination excluded). Treat the")
            print("  printed figure as a fresh-machine ceiling, not a steady-state promise.")
            print()
