...
def find_llama(explicit):
    ...
    raise SystemExit(
        "llama.cpp binaries not found: pass --llama-dir, set QUANTPROBE_LLAMA_DIR, or add to PATH"
    )
...
class _Progress:
    ...
    def step(self, label):
        ...
        print(
            f"\n[quantprobe] step {self.done}/{self.total}: {label}  [elapsed {fmt_dur(el)}{eta}]",
            flush=True,
        )
...
def sh(cmd, dry, capture=False):
    ...
    if capture:
        return (
            ...
            + subprocess.run(cmd, capture_output=True, text=True, check=False).stderr
            if False
            else subprocess.run(
...
def _ppl_once(perp, gguf, eval_file, chunks, ngl):
    p = subprocess.run(
        [perp, "-m", gguf, "-f", eval_file, "--chunks", str(chunks), "-ngl", str(ngl)],
        capture_output=True,
        text=True,
        errors="replace",
        check=False,
...
def kld(perp, gguf, ref_logits, eval_file, chunks, ngl, dry):
    ...
    if not r:
        print(
            "  KL divergence produced no parseable block - reporting UNMEASURED, "
            "NOT substituting perplexity.",
            flush=True,
        )
    return r
...
def ppl(perp, gguf, eval_file, chunks, ngl, dry):
    print(
        f"  measuring perplexity on {chunks} chunks (this can take 1-3 min; llama.cpp is quiet while it works)...",
        flush=True,
    )
    if dry:
        return None
    ...
    if val is None and ngl != 0 and ("out of memory" in out.lower() or "failed to" in out.lower()):
        ...
        print(
            "  GPU offload failed to fit (likely VRAM); retrying at -ngl 0 (CPU, slower)...",
            flush=True,
        )
    ...
    if val is None:
        ...
        print(
            f"  perplexity produced no parseable result. Last output lines:\n{tail}",
            flush=True,
        )
    return val
...
def run(a):
    ...
    print(
        f"quant-probe: {os.path.basename(a.gguf)} | {L} layers -> {len(bands)} bands {bands}",
        flush=True,
    )
    ...
    print(
        ...
        f"{len(bands) + 1} perplexity runs on a {src_gb:.0f} GB source)",
        flush=True,
    )
    if spills:
        print(
            f"  WARNING: the {src_gb * 0.66:.0f} GB working file exceeds your memory, so every\n"
            f"  perplexity run pages from disk - that is what makes this slow. A smaller source\n"
            f"  (or more RAM) changes this from hours to minutes.",
            flush=True,
        )
    print(
        "  (measured on the reference box; refined from YOUR pace after step 1. "
        "Ctrl-C is safe between steps.)",
        flush=True,
    )
    if est > 120 and not getattr(a, "yes", False) and not a.dry_run:
        try:
            if (
                input(f"\n  This will take about {fmt_dur(est)}. Continue? [y/N]: ").strip().lower()
                != "y"
            ):
                raise SystemExit(
                    "aborted - nothing was built. Re-run with --yes to skip this prompt."
                )
        except EOFError:
            raise SystemExit(
                f"this probe needs ~{fmt_dur(est)} and there is no terminal to confirm.\n"
    ...
    if any(d is None for d in deltas):
        ...
        print(
            "  INCOMPLETE: one or more bands produced no perplexity - the curve cannot be "
            "trusted and no recipe is emitted. See the errors above.",
            flush=True,
        )
        raise SystemExit(1)
    ...
    lo, hi = bands[worst]
    print(
        f"  fragile band: layers {lo}-{hi} (delta +{deltas[worst]:.2f} vs "
    ...
    if getattr(a, "apply", False):
        ...
        if imat == "auto":
            imat = make_imatrix(
                ...
                a.gguf,
                a.eval,
                chunks=getattr(a, "imatrix_chunks", 100),
                ngl=a.ngl,
                dry=a.dry_run,
            )
        ...
        print(
            "\n  (re-run with  --apply --out model-2bit.gguf  to BUILD this GGUF automatically)",
            flush=True,
        )
...
def build_depthaware(
    ...
    cmd += [
        "--tensor-type",
        f"{_band_re(protect_lo, protect_hi)}={protect}",
        "--tensor-type",
        "attn_.*=q4_k",
        "--tensor-type",
        "ssm_.*=q4_k",
    ...
    print(
        f"[quantprobe] building depth-aware GGUF: protect layers {protect_lo}-{protect_hi} @ {protect}"
    )
    print("  $ " + " ".join(cmd))
    if dry:
        return out
    ...
    if rc == 0 and os.path.exists(out):
        print(
            f"[quantprobe] done -> {out} ({os.path.getsize(out) / 1e9:.2f} GB). "
...
def make_imatrix(llama_dir, src, eval_file, out=None, chunks=100, ngl=99, dry=False):
    ...
    print(
        f"[quantprobe] building importance matrix over {chunks} chunks on a {src_gb:.0f} GB source"
    )
    print(
        f"  ESTIMATED TIME: ~{fmt_dur(im_est)} (measured 4.5 h for a 35 GB source at 100 chunks)."
    )
    print("  Worth ~8% quality at zero size/speed cost - but it is a real time commitment.")
    print("  Skip it with --no-imatrix; reduce it with --imatrix-chunks (quality scales down too).")
    print("  $ " + " ".join(cmd))
    if dry:
        return out
    rc = subprocess.call(cmd)
    if rc != 0 or not os.path.isfile(out):
        print(
            f"[quantprobe] imatrix generation failed (exit {rc}); continuing WITHOUT calibration."
        )
        return None
...
def quantize(a):
    ...
    if not os.path.isfile(a.gguf):
        raise SystemExit(
            f"GGUF not found: {a.gguf}  (point --gguf at a real high-precision GGUF: f16/bf16/Q8)"
        )
    ...
    if a.protect:
        ...
        if not r:
            raise SystemExit(
                f"no recipe '{rec_key}'. See what has been measured: quantprobe recipes"
            )
        ...
        print(
            f"[quantprobe] using measured recipe '{rec_key}': protect layers {lo}-{hi} "
            f"({r['probe']['shape']}-fragile, {r['probe']['fragility_ratio']}x median)"
        )
        print(
            f"  measured {r['provenance']['measured']} on {r['provenance']['hardware']}; "
        ...
        try:
            ...
            if r:
                ...
                if (rlo, rhi) != (lo, hi):
                    print(
                        f"[quantprobe] a MEASURED recipe exists for this model class "
