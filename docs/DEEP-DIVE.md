...
| **vs llama.cpp** (Qwen3-30B, same GGUF, same box, one session — pre-registration #65) | naive default: 7.45 tok/s · informed `-ngl 20`: 19.29 | planned expert split: **20.78** | **2.8× vs naive, +8% vs informed** |
...
The llama.cpp row is the [MACHINE_LADDER.md](../MACHINE_LADDER.md) protocol — one session,
pristine binary, clocks logged, predictions staked before any measurement. One honesty note it
carries (pre-registration #60): a hand-tuned fixed `-ngl` split matches the `-ot` placement on
raw generation; the split earns its keep on prompt processing (2.2× measured), on keeping KV in
VRAM, and on enabling speculation.
...
| [MACHINE_LADDER.md](../MACHINE_LADDER.md) | every model four ways — naive default / informed llama.cpp / quantprobe / staked prediction — with the v1.20.2 accuracy correction |
