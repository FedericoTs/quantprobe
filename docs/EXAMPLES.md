...
[quantprobe] no hardware flags: auto-detected this machine
  *   19.5 tok/s  split experts: 19%->VRAM, rest->RAM
      17.5 tok/s  hybrid: attention->VRAM, experts->RAM   [RAM boundary - needs --no-mmap]
       3.1 tok/s  stream from disk (cold experts)   [exceeds RAM - capacity demo]
```

Two measurements against those two rows, on the same box:

- **hybrid, 17.5 predicted → 18.32 ± 0.17 measured** (+4.7% conservative). This is the original
  validation and it still reproduces byte-for-byte with today's code — the law has not moved.
- **split experts, 19.6 predicted → 19.22 ± 0.7 measured** (−2%). This row did not exist until
  v1.8.0; the planner now recommends it because it is faster on the same file.

The `*` moved from hybrid to split experts because the tool learned a better placement, not
because the prediction for hybrid changed. Both are checked in the release gate.
