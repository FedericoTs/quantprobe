...
| 15 | **Attack Law 3** (U-46) — Law 3 says fragility is *"measurable, not predictable"*, but it only ruled out STATIC signals (architecture family, weight kurtosis) and never tested a cheap FUNCTIONAL one. Does per-layer **effective rank**, from forward passes with no quantization at all, call the fragile band the 30-minute probe finds? Ground truth is our own atlas. Scored on SHAPE only, **Mistral held out** (the 27× outlier that breaks every other rule). If it lands, the probe collapses to a forward pass and `quantize` works on unseen models; if it fails, Law 3 is stronger for having survived the exclusion it never tested. n=4 is thin — the stake must fix statistic and rule *before* any band delta is looked at | CPU-light | A |
| 16 | **Cross-model KV transfer** (E-19, U-44/U-45) — U-44: we hold a matched-KV pair (7B↔30B, `kv_heads=4 head_dim=128`) that spans generations AND a dense/MoE boundary, which arXiv:2608.03893 does not test — is the linear structure a family property or does geometry suffice? U-45: the same idea across two QUANTIZATIONS of one model, where the mapper may be the identity — prefill on the compute-friendly quant, decode on the bandwidth-friendly one, which only L-12 + prereg #55 together suggest | CPU-heavy | A |
| 14 | **Width vs depth** (prereg #96, E-18) — at a FIXED token budget, does 16 parallel candidates or 1 candidate + sequential execution-grounded repair buy more? Credit **[@danpacary](https://x.com/danpacary/status/2085794035197960418)** for the repair arm and the "same exam, only the harness changed" design. P-1 is staked AGAINST us: if depth wins, the lanes framing is wrong about *why* it works and the P0 posts get amended at full prominence. Ships a decision rule if P-3 holds — few failing tests → repair, many → resample | evening + GPU | A |
