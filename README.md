...
- **You want a tight number for a model that fits entirely in VRAM.** This is the placement the law knows least well: across all seven models measured there, the real speed came in **faster than predicted every time — by 2% to 67%.** Read that row as a floor, not a ceiling. Seven points on one GPU don't identify a better formula, and we'd rather say so than move a constant on thin evidence. **This is the single most useful thing you can send us:** `quantprobe bench --contribute` on a GPU-resident model turns your machine into the datapoint that fixes it.
