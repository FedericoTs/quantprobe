...
### L-20 — Effective decode bandwidth is a property of FORMAT x TENSOR SHAPE, not of format alone. Measured on one card with everything else held constant (prereg #80): 4.5-bit weights reach 100.5 GB/s at 16384 rows/tensor and 31.0 GB/s at 128 - monotone across eight points, 3.2x end to end. The knee is around 4096-8192 rows, where the curve saturates at the format's true ceiling. CONSEQUENCE: every FORMAT_EBW entry quantprobe ships was measured on dense-7B FFN-shaped streams and is therefore an UPPER BOUND that only large homogeneous matrices reach; applying it to attention-shaped tensors over-promises by ~23%. This is the same physics as L-19 (CPU attention at depth) and #59 (the small-model size floor) at a third granularity: work that is small per launch does not reach the bandwidth its byte count suggests.

**Magnitude:** 3.2x bandwidth spread by shape alone; attention-shaped -23% vs FFN-shaped

`established` · `measured` · scope: Pascal, batch-1 decode geometry (one output row per block, #55); the SHAPE of the curve is the claim, the knee position is per-card and unmeasured elsewhere · evidence: prereg #80 (tools/kernelprobe/shape.cu, weights/data/prereg80_shape.log); explains prereg #79's failure (r=0.87 vs attention share) · wired into: `findings only - NOT yet in the tool: shape-classing FORMAT_EBW requires U-29's read-set weights first, or it repeats #79's failure with better constants`
