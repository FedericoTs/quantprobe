...
`open` · `suggestive` · scope: Pascal, six GPU-resident models. Correlation on n=6 with one custom model (gemma4-late12) in it - suggestive only. · evidence: weights/shape_vs_ops.py; L-20 curve from prereg81_knee.log (single session, C-14 safe) · wired into: `nothing yet. The honest next step is a STAKED prereg: predict each model's achieved-bandwidth fraction from geometry BEFORE measuring a fresh ladder, with a kill rule on LOO beating 8.7%.`
