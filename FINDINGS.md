...
`established` · `measured` · scope: Pascal 6GB, qwen3moe 30B.A3B Q2_K, exps=CPU placement, same binary/session as staked. Dense-in-VRAM control VIOLATES the form (prereg #19 P-2 collapses at ub2048) - CPU-expert tier only. · evidence: weights/data/prereg92b_ub_oos.log (predictions staked in the log header before the run); fit from weights/data/prereg19_ubatch.log · wired into: `plan.py MoE prefill advice should emit -b 4096 -ub 4096 for this placement - integration owed`
