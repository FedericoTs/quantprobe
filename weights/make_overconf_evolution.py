...
54.7/55.7 split is a stable property, not endpoint noise. The third curve is B4b's repair-loop v2: post-repair failure rate.
"""
...
def curves():
    f1, f2, f3 = [], [], []
    f1_final, f2_final, f3_final = [False], [False], [False]
    for line in open(LOG, encoding="utf-8"):
        ...
        if m:
            ...
            if den <= 0:
                ...
                f3.append((n, 100 * d["ref_failed"] / den))       # v2: POST-repair failures
            else:
        ...
        m = re.search(r"feed2: kept (\d+), dropped (\{.*\})", line)
        if m and "repaired" in m.group(2) and int(m.group(1)) >= 100 and not f3_final[0]:
            d = ast.literal_eval(m.group(2))
            den = 500 - d["parse"] - d["no_fn"] - d.get("gen_fail", 0)
            f3.append((500, 100 * d["ref_failed"] / den)); f3_final[0] = True
    f1.sort(); f2.sort(); f3.sort()
    return f1, f2, f3
...
def main():
    ...
    for pts, col, lw, dash in ((f1, brand.VRAM, 5, ""), (f2, TEAL, 5, ' stroke-dasharray="14,10"'),
                               (f3, brand.DISK, 5, ' stroke-dasharray="4,7"')):
        if not pts:
            continue
    ...
    if f3:
        n3, r3 = f3[-1]
        s.append(f'<text x="{X(n3)+24}" y="{Y(r3)+12}" fill="{brand.DISK}" font-size="24" font-weight="bold">30B + one repair pass - {r3:.1f}%</text>')
        s.append(f'<text x="{X(n3)+24}" y="{Y(r3)+40}" fill="{SUB}" font-size="18">re-reading recovered 7.3% of failures - only execution finds wrong literals</text>')
    s.append(f'<text x="{px0+14}" y="{py1-30}" fill="{INK}" font-size="29" font-weight="bold">scale does not fix it, introspection barely dents it - execution does</text>')
    s += [brand.footer(W, H, "parsed from weights/data/phaseb_gen.log checkpoints - three arms, one log, nothing smoothed"),
          '</svg>']
