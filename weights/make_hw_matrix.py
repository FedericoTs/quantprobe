...
def main():
    ...
    print("Its four presets are a genuine out-of-sample test, and they **discriminate**. Kimi K3")
    print("routes 16 of 896 experts per token; solving the 1.56 TB checkpoint for the expert/trunk")
    print("split gives a 1.33 TB expert store, so **23.8 GB moves per token** and it moves from")
    print("disk in every preset:\n")
    print("| preset | RSS | reported | implied effective read BW |")
    print("|---|---|---|---|")
    print("| laptop | 8.2 GB | 32.69 s/tok | 0.73 GB/s |")
    print("| desktop | 31.9 GB | ~29.5 s/tok | 0.81 GB/s |")
    print("| workstation | 95.5 GB | ~24 s/tok | 0.99 GB/s |")
    print("| server | ~128 GB | ~20 s/tok | 1.19 GB/s |\n")
    print("**Law 4 predicts** bytes/token is constant across all four, because which 16 of 896")
    print("experts a token needs changes every token - the working set over a generation is the")
    print("whole 1.33 TB, and no preset caches a meaningful slice of it. So adding RAM should buy")
    print("almost nothing. **The obvious rival model** - *it is slow because it does not fit,")
    print("add RAM* - predicts speed tracks resident set: 8.2 GB to 128 GB is 15.6x.\n")
    print("**Measured: 1.63x.** Far nearer \"nearly nothing\" than 15.6x. That is the tiered model's")
    print("whole point - when the binding resource is streaming bandwidth against a store you")
    print("cannot cache, memory *capacity* is not the lever. (The implied bandwidths above are")
    print("derived from the measurements, so that column is a consistency check with one free")
    print("parameter per row; the 1.63x-vs-15.6x discrimination uses none, and is the real result.)\n")
    print("To actually hit the 10 tok/s the relay claimed you need 238 GB/s sustained against a")
    print("1.33 TB store. A Spark has the bandwidth and 8.6% of the store; an H100 has 3350 GB/s")
    print("and 6.0%. You need ~16x H100 before the store is resident - at which point the C is")
    print("irrelevant. No single box gets there.\n")
