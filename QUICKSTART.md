...
**Best possible custom quality.** `auto ... --custom` now also builds an **importance matrix** —
a calibration pass that measures which weights actually carry signal, so precision goes where it
matters. Measured **−8.5% perplexity at ~3 bits, for zero extra file size and no speed cost**
(pre-registration #12). It adds one pass to the build; skip it with `--no-imatrix` if you're in a
hurry. Doing it by hand:

```bash
quantprobe probe --gguf model-f16.gguf --eval wiki.test.raw --imatrix auto --apply --out model-2bit.gguf
```

Calibrate on text that is **not** your eval text — calibrating on what you score against inflates
the result. (We used wikitext *train* against a wikitext *test* eval, and disclosed even that as
in-domain.)
